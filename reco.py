...
class ChronicleReco:
  ...
  """
    ...
    """
        for ide, name in items.items():
            ...
            for i in range(ide + 1, self.__size):
                try:
                    if (self.items[i] == name) and (
                            name[1] == 0):  # avoid to find same event if two same items in chronicle
    ...
    """
        ...
        new_occs_with_itemsname = [[(self.items[item_num][0], date) for item_num, date in enumerate(new_occ)] for
                                   new_occ in new_occs]
            ...
            for eiej in futur_tconst:  # remind ei is item_index
                ...
                if 0 in self.tconst[eiej]:  # check if positive edge
                    ...
                    if (tmin > tmax) or (tmax > dmax) or (tmin < 0):  # check if interval is valid
                        ...
                        try:
                            dates_ck = set(
                                sequence[nj]) & interval1  # inter btw item occurrence date and given interval
                        except:
                            dates_ck = set()
                        if not dates_ck:
                            occ[ej] = {1: (tmin, tmax)}
    ...
    """
        ...
        for occ in occs:
            ...
            if 0 in occ[item_index]:
                ...
                try:
                    dates_ck = set(
                        sequence[ck]) & interval0  # All dates from occurence date of ck in the given interval0
                    satisfaible = True
                except:
                    dates_ck = set()
                    satisfaible = False
